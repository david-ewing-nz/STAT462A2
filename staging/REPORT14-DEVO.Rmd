...
make_conf_matrix <- function(pred_class, actual_class) {
  pred_class <- if (is.numeric(pred_class)) factor(pred_class, levels = c(0, 1), labels = c("LIVE", "DEAD")) else pred_class
  actual_class <- if (is.numeric(actual_class)) factor(actual_class, levels = c(0, 1), labels = c("LIVE", "DEAD")) else actual_class
  table(
...
make_colour_conf_matrix <- function(pred_class, actual_class) {
...
eval_colour_cmtx <- function(cmtx) {
  if (!all(sort(rownames(cmtx)) == c("BLUE", "GREEN", "RED")) ||
      !all(sort(colnames(cmtx)) == c("BLUE", "GREEN", "RED"))) {
    stop("Confusion matrix must use levels: RED, GREEN, BLUE")
  }

  total <- sum(cmtx)
  if (total == 0) return(rep(NA, 3))
  ...
  for (i in seq_along(labels)) {
    label <- labels[i]
    tp <- cmtx[label, label]
    fp <- sum(cmtx[, label]) - tp
    fn <- sum(cmtx[label, ]) - tp
  ...
  c(misclass, accuracy, macro_precision, macro_recall)
}
