...
set.seed(123)
cp_split <- initial_split(cp_clean, prop = 0.8, strata = color)
...
c_plot_dist <- ggplot(cp_clean, aes(x = r, y = b, color = color)) +
  geom_point(alpha = 0.6) + labs(title = "color Classes by R and B")
 
c_plot_train <- ggplot(cp_train, aes(x = r, y = b, color = color)) +
  geom_point(alpha = 0.6) + labs(title = "color train")
c_plot_test <- ggplot(cp_test, aes(x = r, y = b, color = color)) +
...
cp_model <- multinom(c_equation_lg, data = cp_train, family = "multinomial")
list_of_df <- pretty_multinom(cp_model)
...
print(pretty_cm(cp_cmtx_glm, title = "GLM Confusion Matrix"))
...
cp_qda_model <- qda(c_equation_lg, data = cp_train)
...
print(pretty_cm(cp_cmtx_qda, title = "QDA Confusion Matrix"))
