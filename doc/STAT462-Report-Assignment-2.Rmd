...
#  Select model-relevant columns from heart.csv ----
...
select_model_features <- \(df) {
  df %>%
    ...
    # Remove known metadata or timing/event flags (often not predictive)
    select(-any_of(c("RANDID", "PERIOD", "TIMECVD", "TIMEMIFC", "TIMECHD", 
                     "TIMEDTH", "TIME", "TIMEMI", "TIMESTRK", "TIMESCHD", 
                     "TIMEAP"))) %>%
    
    # Keep only columns used in assignment instructions
    select(any_of(c("age", "sex", "sysbp", "diabp", "glucose", "diabetes", 
                    "bmi", "educ", "cvd")))
}

#  Summarise Selected Model Features ----

summarise_model_features <- \(df) {
  df %>%
    skimr::skim() %>%
    skimr::skim_select(numeric) %>%  # Focus only on numeric features
    skimr::skim_to_wide() %>%
    flextable::flextable() %>%
    flextable::autofit() %>%
