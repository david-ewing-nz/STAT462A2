...
```{r wsb-env-flags, include```=FALSE}
# Detect if output is latex 
...
```{r wsb-include-helper, child = helper_path, eval = !is_pdf}
```
...
hp_actual_class <- factor(hp_actual_class, levels = c("LIVE", "DEAD"))
...
hp_cmtx_10 <- table(Predicted = hp_pred_class_10, Actual = hp_actual_class)
```  
...
hp_qda_pred <- predict(hp_qda_model, newdata = hp_test)
...
# Build QDA model using Heart training data
hp_qda_model <- qda(DEATH ~ GLUCOSE + SYSBP + AGE, data = hp_train)
...
```{r heart-wsb-step-13}
#cat("### Tree-Based Model Evaluation (Heart Data)\n")

# Predict class labels using the tree model
hp_tree_pred_class <- predict(hp_tree_model, newdata = hp_test, type = "class")

# Define actual class labels for test set
hp_actual_class <- factor(hp_test$DEATH, levels = c("LIVE", "DEAD"))

# Create confusion matrix and evaluate
hp_tree_cmtx <- make_conf_matrix(hp_tree_pred_class, hp_actual_class)
hp_tree_metrics <- eval_conf_matrix(hp_tree_cmtx)
hp_tree_mtrx_df <- summarise_metrics(hp_tree_metrics)

# Format confusion matrix for display
hp_tree_cmtx_df <- as.data.frame.matrix(hp_tree_cmtx)
hp_tree_cmtx_df <- cbind(Predicted = rownames(hp_tree_cmtx_df), hp_tree_cmtx_df)
rownames(hp_tree_cmtx_df) <- NULL

# Create formatted flextables
pretty_hp_tree_cmtx <- pretty_df(hp_tree_cmtx_df, title = "Tree-Based Confusion Matrix (Heart Data)")
pretty_hp_tree_metrics <- pretty_df(hp_tree_mtrx_df, title = "Tree-Based Performance Metrics (Heart Data)")

``` 

```{r heart-wsb-step-14}
#cat("### Compare Model Performance (All Models)\n")

model_results <- list(
  "Logistic (0.5)" = list(actual = hp_actual_class, predicted = hp_pred_class),
  "Logistic (0.1)" = list(actual = hp_actual_class, predicted = hp_pred_class_10),
  "QDA"            = list(actual = hp_actual_class, predicted = hp_qda_pred_class),
  "Tree"           = list(actual = hp_actual_class, predicted = hp_tree_pred_class)
)
...
comparison_ft <- pretty_model_comparison(model_results)
