...
```{r child="../doc/WSB-heart-analysis-v4.Rmd" , cache = FALSE}
```
...
# ---------------------------
# Combined WBS Child Content
# ---------------------------


```{r setup-heart, include=FALSE}
...
## WBS for Heart Analysis
```{r functions}
confusion_mtx <- function(conf_mtx) {
 


#   percent matrix safely
if (sum(mat) == 0) {
  mat_percent <- matrix(0, nrow = nrow(mat), ncol = ncol(mat))
} else {
  mat_percent <- round(100 * mat / sum(mat), 1)
}
dimnames(mat_percent) <- dimnames(hp_cmtx)  #  row/col names
# Step 2: Convert to character matrix with %
matp_text <- matrix(paste0(mat_percent, "%"), 
                       nrow = nrow(mat_percent), 
                       dimnames = dimnames(mat_percent))
cat("mat_percent = ",mat_percent,"\n")

#   3×3 data frame
labels <- rownames(mat_text)
conf_3x3 <- matrix("", nrow = 3, ncol = 3)

# Insert percentage matrix into [2:3, 2:3]
conf_3x3[2:3, 2:3] <- matp_text

# Add axis labels
conf_3x3[1, 2:3] <- paste0("Actual: ", labels)
conf_3x3[2:3, 1] <- paste0("Predicted: ", labels)

# Convert to data frame
hp_cmtx_df <- as.data.frame(conf_3x3)
colnames(hp_cmtx_df) <- NULL
...
hggp = ggplot(hp_clean, aes(x = GLUCOSE, y = SYSBP, colour = DEATH)) +
  geom_point(alpha = 0.6) +
  labs(title = "DEATH vs GLUCOSE and SYSBP", x = "GLUCOSE", y = "SYSBP")
hggp


### Visualise distributions and log/linear axis combinations

# 1. Distribution of SYSBP
p_sysbp <- ggplot(hp_test, aes(x = SYSBP)) +
  geom_histogram(bins = 40, fill = "steelblue", colour = "white") +
  labs(title = "Distribution of SYSBP", x = "SYSBP", y = "Count")

# 2. Distribution of GLUCOSE
p_glucose <- ggplot(hp_test, aes(x = GLUCOSE)) +
...
pretty_ggplot(hggp, "DEATH, GLUCOSE, and SYSBP" )
```
...
hp_pred_class <- factor(hp_pred_class, levels = c("LIVE", "DEAD"))

# Define actual class labels with fixed levels
...
# Summarise into a printable data frame
...
# Updated: compute evaluation and show summaries
eval_results <- tryCatch({
  model_boolean_eval(hp_glm_metrx, hp_pred_class, hp_actual_class, hp_train)
}, error = function(e) {
  message("⚠️ model_boolean_eval() failed: ", conditionMessage(e))
  return(list(boolean_eval = NULL, metrics_eval = NULL))
})

if (is.data.frame(eval_results$boolean_eval)) {
  print(pretty_df(eval_results$boolean_eval, title = "Model Evaluation - Qualitative Booleans"))
} else {
  cat("⚠️ Skipping qualitative evaluation — not a data frame.\n")
}

if (is.data.frame(eval_results$metrics_eval)) {
  print(pretty_df(eval_results$metrics_eval, title = "Model Evaluation - Quantitative Metrics"))
} else {
  cat("⚠️ Skipping quantitative evaluation — not a data frame.\n")
}
```
...
```{r wsb-step-06}
hp_cmtx_test = table(Predicted = hp_pred_class, Actual = hp_actual_class)
```

### Visualise decision boundaries in the GLUCOSE–SYSBP plane
```{r wsb-step-07}
library(ggplot2)
ggplot(h_test, aes(x = GLUCOSE, y = SYSBP, colour = as.factor(h_pred_class))) +
  geom_point(alpha = 0.5) +
  labs(title = "Logistic Regression Decision Boundary (approx)", colour = "Predicted")
```

### Comment on the goodness of fit

### Supporting numeric metrics for evaluation criteria
```{r wsb-step-8.0, include=FALSE}
cat("### Supporting numeric metrics for evaluation criteria\n")

# Format p-values to avoid misleading zeros
h_min_pval  <- format.pval(min(h_pvals), digits = 3, eps = .Machine$double.eps)
h_mean_pval <- format.pval(mean(h_pvals), digits = 3, eps = .Machine$double.eps)

# Other metrics
h_delta_acc      <- h_test_acc - h_base_acc
h_train_test_gap <- abs(h_train_acc - h_test_acc)

# Construct table
hp_model_eval_metrics <- data.frame(
  Metric = c(
    "Train Accuracy",
    "Test Accuracy",
    "Baseline Accuracy",
  ...
  Value = c(
    ...
    round(h_test_acc, 5),
    round(h_base_acc, 5),
    round(h_test_acc - h_base_acc, 5),
    round(abs(h_train_acc - h_test_acc), 5),
    h_min_pval,
    h_mean_pval
  ),
  Variable = c(
    ...
    "h_test_acc",
    "h_base_acc",
    "h_test_acc - h_base_acc",
    "abs(h_train_acc - h_test_acc)",
    "h_min_pval",
    "h_mean_pval"
  ),
  stringsAsFactors = FALSE
...
```{r wsb-step-8.01}
#print(pretty_df(hp_model_eval_metrics,"foo"))
pretty_df(hp_model_eval_metrics,"FOO")

```

```{r wsb-step-08.1}
print("The logistic regression model shows significant predictors, but fit is moderate. Improvements may come from non-linear methods.")
```
### Evaluate model performance: generate boolean summary table
### Evaluate model performance: generate boolean summary table
```{r wsb-step-08.2, include=FALSE}
cat("### Evaluate model performance: generate boolean summary table\n")

# Step 1: Compute result variables


 bar <- \(pvals, test_acc, base_acc, train_acc) {
      foo <- list ( 
      sig_all                       <- all(pvals < 0.05),
      better_than_random            <- test_acc > 0.5,
      better_than_baseline_mild     <- (test_acc - base_acc) >= 0.01,
...
x_glm_metrx <- glm(DEATH ~ GLUCOSE + SYSBP + AGE, data = hp_train, family = "binomial")  
  
  print(length(x_glm_metrx))
 
 xpred_probs <- predict(x_glm_metrx, newdata = hp_test, type = "response") #


 xpred_class <- predict_classes(xpred_probs, threshold = 0.5)

print(length(xpred_class))
print(length(xpred_probs))

 xpvals      <- broom::tidy(hp_glm_metrx)$p.valuue
 
 if (length(xpred_class) != length(h_actual_class)) {
  stop("Length mismatch: predicted = ", length(xpred_class), 
       ", actual = ", length(h_actual_class))
       
}

print(length(h_actual_class))
print(length(xpred_class))

xtest_acc  <- mean(xpred_class == h_actual_class)
print(xtest_acc)

xbase_acc  <- max(prop.table(table(h_actual_class)))

xtrain_acc <- mean(h_train_class == h_train$DEATH) 

xxx <- bar( xpvals, xtest_acc, xbase_acc, xtrain_acc)
...
```{r wsb-step-08.3, }
#print(pretty_df(hp_model_eval))
pretty_df(hp_model_eval)
```

### Modify threshold: classify as 'risky' if risk > 10%
```{r wsb-step-09}
...
h_cmtx_10 = table(Predicted = h_pred_class_10, Actual = h_actual_class)
```
...
library(MASS)
h_qda_model <- qda(DEATH ~ GLUCOSE + SYSBP + AGE, data = h_train)
h_qda_pred <- predict(h_qda_model, newdata = h_test)$class
mean(h_qda_pred != h_actual_class)
h_cmtx_qda = table(Predicted = h_qda_pred, Actual = h_actual_class)
```
...
h_qda_probs <- predict(h_qda_model, newdata = h_test)$posterior[, 2]
ggplot(h_test, aes(x = GLUCOSE, y = SYSBP, colour = h_qda_pred)) +
  geom_point(alpha = 0.5) +
  labs(title = "QDA Prediction by GLUCOSE and SYSBP", colour = "Predicted")
```

### Identify strong risk factors and communicate results
```{r wsb-step-13}
summary(hp_glm_metrx)
print("GLUCOSE, SYSBP and AGE appear as key risk indicators based on logistic regression coefficients.")
```
