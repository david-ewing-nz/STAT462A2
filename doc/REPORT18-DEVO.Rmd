...
run_heart_eval <- function(model, threshold, type = "GLM" ) {
  ...
  probs <- switch(         #  model type from predicted probabilities
    type,
    ...
    "QDA"  = predict(model, newdata = hp_test)$posterior[, "DEAD"],  # ✅ Extract only the probabilities
  
    stop("Unsupported model type")
  )

  pred_class <- predict_heart_classes(probs, threshold)     # Predictx class from prob
  cmtx_tbl   <- make_heart_cmtx(pred_class, hp_actual_class)# Confusion matrix 
  metrics_df <- eval_heart_cmtx(cmtx_tbl) |>                  # and metrics
                  
  title      <- glue("Confusion Matrix: {type}, Threshold = {threshold}")
  cmtx_ft    <- pretty_cmtx( cmtx_tbl, title)
  
  title      <- glue("Performance: {type}, Threshold = {threshold}")        
  metrics_df <- eval_heart_cmtx(cmtx_tbl) |> # make pretty
                  mutate(Threshold = as.character(threshold)) |>
                  relocate(Threshold, .before = Metric)

  misclass <- metrics_df$Value[metrics_df$Metric == "Misclass Rate"]   # Misclass  

  # Return all useful outputs
  list(
    pred_class = pred_class,
    cmtx_tbl   = cmtx_tbl,
    cmtx_ft    = cmtx_ft,     #if you need to do more
...
hp_equation_lg <- DEATH ~ GLUCOSE + SYSBP + AGE
hp_model <- glm(hp_equation_lg, data = hp_train, family = binomial)

# Predict probabilities and classify using threshold = 0.5
hp_pred_probs_50 <- predict(hp_model, newdata = hp_test, type = "response")
hp_pred_class_50 <- predict_heart_classes(hp_pred_probs_50, threshold = 0.5)

# Build confusion matrix and summarise
hp_cmtx_50_tbl <- make_heart_cmtx(hp_pred_class_50, hp_actual_class)
hp_cmtx_50_ft  <- pretty_cmtx(hp_cmtx_50_tbl, "Logistic Regression Confusion Matrix (Threshold = 0.5)")

# Derive performance metrics
hp_mtrx_50_df  <- eval_heart_cmtx(hp_cmtx_50_tbl)
hp_mtrx_50_ft  <- pretty_df(hp_mtrx_50_df, "Logistic Regression Performance (Threshold = 0.5)")

# Preview numeric result for report
misclass_rate_50 <- hp_mtrx_50_df$Value[hp_mtrx_50_df$Metric == "Misclass Rate"]

# Print both for verification (in dev mode)
print(hp_cmtx_50_ft)
print(hp_mtrx_50_ft)
print(glue("Misclassification Rate: {misclass_rate_50}"))
