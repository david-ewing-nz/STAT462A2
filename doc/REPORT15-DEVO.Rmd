...
pretty_model_comparison <- function(model_list) {
  ...
  for (name in names(model_list)) {
    ...
    cm <- make_conf_matrx(predicted, actual)
...
# Convert binary probabilities to factor labels for heart
predict_heart_classes <- function(probs, threshold = 0.5) {
  pred <- ifelse(probs > threshold, 1, 0)
  factor(pred, levels = c(0, 1), labels = c("LIVE", "DEAD"))
}
...
eval_heart_cmtx <- function(cmtx_tbl) { #returns a dataframe)
  if (!all(rownames(cmtx) == c("LIVE", "DEAD")) || !all(colnames(cmtx) == c("LIVE", "DEAD"))) {
...
summarise_heart_results <- function(pred_probs, actual_class, threshold = 0.5) {
  ...
  metrics_df <- eval_heart_cmtx(cmtx)
  cmtx_df    <- as.data.frame.matrix(cmtx_tbl)
  list( 
    pred_class = pred_class,
    cmtx_ft   = pretty_mtrx(cmtx_tbl),
    mtrx_df   = pretty_mtrx()   metrics_df,
    cmtx_ft   = pretty_cmtx(cmtx),
    mtrx_ft   = pretty_df(metrics_df, title = glue("Performance (Threshold = {threshold})"))
  )
}
...
h_pred_probs          <- predict(h_model, newdata = h_test, type = "response")  # Predict class probabilities on test 
h_pred_class_50    <- predict_heart_classes(h_pred_probs, threshold = 0.5)        # prob -> class labels

h_results_50 = summarise_heart_results(h_pred_probs, h_pred_class_50, threshold = 0.5)  

#pretty_df(h_results_50$pred_class  )
pretty_df(h_results_50$cmtx     )    

pretty_mtrx(h_results_50$mtrx_df  ) 
print(h_results_50$cmtx_ft   ) 
print(h_results_50$metric_ft    )
...
hp_cmtx_test_10 <- make_conf_mtrx(hp_pred_class_10, hp_actual_class)
hp_metrics_10 <- eval_heart_cmtx(hp_cmtx_test_10)
...
hp_cmtx_qda <- make_conf_mtrx(hp_qda_pred_class, hp_actual_class)
hp_metrics_qda <- eval_heart_cmtx(hp_cmtx_qda)
...
hp_cmtx <- make_conf_mtrx(hp_pred_class, hp_actual_class)
hp_metrics <- eval_heart_cmtx(hp_cmtx)
...
c_cmtx_glm <- make_conf_mtrx(c_pred_glm, c_actual_class)
